...
public class ExportService : BackgroundService
{
    ...
    {
        // load from db
        var requests = await exportRequests.AllowFiltering().ExecuteAsync();
    ...
    private async Task RunExport(ExportRequest request)
    {
        ...
        var columnMapping = new Dictionary<string, Func<SaveAuction, string>>() { {
            ...
            {"inInventory", a => buyerLookup.GetValueOrDefault(GetBuyerProfile(a))?.ItemsInInventory?.GetValueOrDefault(a.FlatenedNBT.GetValueOrDefault("uid"), "no").ToString()},
